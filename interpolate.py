...
class NaturalCubicSpline:
    """Calculates the natural cubic spline approximation to the batch of datapoints given. Missing values are supported.
    Also calculates the derivative of the cubic spline.
    ...
    def __init__(self, t, X, **kwargs):
        ...
        Arguments:
            ...
            X: tensor of values, of shape (..., L, C), where ... is some number of batch dimensions, L is some length
                ...
                (batch of) L observations in a C-dimensional real vector space, with L observations. Missing values
        ...
        The result is the unique natural cubic spline f such that
        ```
        f(t[i]) = X[...,  i, :]
        ```
        
        Then f(t) may be evaluated with the `evaluate` method, whilst f'(t) may be evaluated with the `derivative`
        method. Furthermore backpropagation through both the `evaluate` and `derivative` is supported.
        """
        ...
        if torch.isnan(X).any():
            ...
            a, b, two_c, three_d = self._compute_coeffs_with_missing_values(t, X.transpose(-1, -2))
        else:
            # Can do things more quickly in this case.
            a, b, two_c, three_d = self._compute_coeffs(t, X.transpose(-1, -2))
        ...
        self._a = a
        self._b = b
        # TODO: switch to just _c and _d
        self._two_c = two_c
        self._three_d = three_d

    @staticmethod
    def _compute_coeffs(times, path):
    ...
    @classmethod
    def _compute_coeffs_with_missing_values(cls, t, path):
        if len(path.shape) == 1:
            ...
            return cls._compute_coeffs_with_missing_values_scalar(t, path)
        else:
            a_pieces = []
            b_pieces = []
            two_c_pieces = []
            ...
            for p in path.unbind(dim=0):  # TODO: parallelise over this
                a, b, two_c, three_d = cls._compute_coeffs_with_missing_values(t, p)
                a_pieces.append(a)
                b_pieces.append(b)
    ...
    @classmethod
    def _compute_coeffs_with_missing_values_scalar(cls, times, path):
        ...
        a_pieces_no_nan, b_pieces_no_nan, two_c_pieces_no_nan, three_d_pieces_no_nan = cls._compute_coeffs(times_no_nan,
                                                                                                           path_no_nan)
    ...
    def _interpret_t(self, t):
        ...
        fractional_part = t - self._t[index]  # will never access the last element of self._t; this is correct behaviour
